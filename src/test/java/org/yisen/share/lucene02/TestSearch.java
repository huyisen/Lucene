package org.yisen.share.lucene02;
...
import org.yisen.share.lucene02.FileIndexUtils;
import org.yisen.share.lucene02.SearcherUtil;
...
public class TestSearch {
	...
	public void copyFile(){
		File file = new File("./lucene/doc");
		File[] files = file.listFiles();
		for (File f : files) {
				try {
					System.out.println("=====> name : "+f.getPath().substring(0, f.getPath().lastIndexOf(".")+1));
					FileUtils.copyFile(f, new File(f.getPath().substring(0, f.getPath().lastIndexOf(".")+1)+"she"));
				} catch (IOException e) {
					e.printStackTrace();
				}
		}
	...
	public void searchByTerm() {
		su.index();
		su.searchByTerm("content", "i", 10);
	}

	@Test
	public void searchByTermRange() {
		// 查询name以a开头和s结尾的
		// su.searchByTermRange("name","a","f",10);
	...
	public void searchByNumRange() {
		su.searchByNumricRange("attach", 3, 10, 10);
	}
	...
	public void searchByWildcard() {
		...
		// 匹配j开头的有三个字符的name
		su.searchByWildcard("name", "j?k?", 10);
	}
	...
	public void testSearchPage01() {
		//su.searchByPage("java",7,10);
		//System.out.println("-------------------------------");
		su.searchByAfter("java",1,20);
