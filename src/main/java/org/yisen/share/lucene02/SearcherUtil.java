...
public class SearcherUtil {
	...
	public SearcherUtil() {
		try {
			directory = FSDirectory.open(new File("./lucene/index03"));
			setDates();
	...
	public void searchByTerm(String field,String name,int num) {
		try {
			...
			printResult(searcher, tds);
		} catch (IOException e) {
			e.printStackTrace();
		}
	}

	private void printResult(IndexSearcher searcher, TopDocs tds) throws CorruptIndexException, IOException {
...
//			query.add(new Term("content","football"));
	...
	public void searchByPage(String field, int pageIndex, int pageSize) {
		try {
			...
			for (int i = start; i < end; i++) {
				Document doc = searcher.doc(sds[i].doc);
				System.out.println(sds[i]+" >> "+ doc.get("filename"));
			}
			searcher.close();
		} catch (Exception e) {
			e.printStackTrace();
	...
	public void searchByAfter(String string, int i, int j) {
